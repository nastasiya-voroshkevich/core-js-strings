...
function concatenateStrings(value1, value2) {
  if (value1 && value2) return value1.concat(value2);
...
function removeLastOccurrences(str, value) {
  ...
  if (str.includes(value)) {
    valueArray.map((i) => array.splice(array.lastIndexOf(i), 1));
  }
  return array.join('');
}
...
function formatTime(minutes, seconds) {
  ...
  return newMinutes.padStart(2, '0').concat(':', newSeconds.padStart(2, 0));
}
/**
...
function countVowels(str) {
  ...
  return array.filter((char) => vowels.includes(char)).length;
}
...
function findLongestWord(sentence) {
  ...
  return sentenceSplit.reduce((longest, currenrWorld) => {
    if (currenrWorld.length > longest.length) return currenrWorld;
    return longest;
  }, '');
}
...
function encodeToRot13(str) {
  ...
  const newAl = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm ?!';
  return str.replace(/[a-z]/gi, (letter) => newAl[al.indexOf(letter)]);
}
